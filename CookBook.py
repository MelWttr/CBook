    ...
    with open(file_name) as f:
        ...
        for line in f:
            ...
            for i in range(ingridient_quantity):
                ...
                ingridients["quantity"] = int(temp_list[1])
...
def get_shop_list_by_dishes(file_name, dishes, person_count):
    cook_book = cook_book_filling(file_name)
    shop_list = {}
    for dish in dishes:
...
def create_shop_list(file_name):
    person_count = int(input('Введите количество человек: '))
    dishes = input('Введите блюда в расчете на одного человека (через запятую): ') \
        .lower().split(', ')
    shop_list = get_shop_list_by_dishes(file_name, dishes, person_count)
    print_shop_list(shop_list)

create_shop_list("Reciepts")
